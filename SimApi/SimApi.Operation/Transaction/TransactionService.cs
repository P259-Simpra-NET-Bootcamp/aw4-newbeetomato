...
public class TransactionService : ITransactionService
{
    ...
    public ApiResponse<List<TransactionResponse>> GetAll()
    {
        try
        {
            var entityList = unitOfWork.Repository<Data.Transaction>().GetAll();
            var mapped = mapper.Map<List<Data.Transaction>, List<TransactionResponse>>(entityList);
    ...
    public ApiResponse<List<TransactionResponse>> GetByParameter(int accountId, int customerId, decimal amount, string description)
    {
        ...
        var list = unitOfWork.Repository<Data.Transaction>().Where(predicate).ToList();

        var mapped = mapper.Map<List<Data.Transaction>, List<TransactionResponse>>(list);
        return new ApiResponse<List<TransactionResponse>>(mapped);
    }
    ...
    public ApiResponse<List<TransactionResponse>> GetByReference(string referenceNumber)
    {
        try
        {
            var entityList = unitOfWork.Repository<Data.Transaction>().Where(x => x.ReferenceNumber == referenceNumber).ToList();
            var mapped = mapper.Map<List<Data.Transaction>, List<TransactionResponse>>(entityList);
            return new ApiResponse<List<TransactionResponse>>(mapped);
        }
        catch (Exception ex)
    ...
    public ApiResponse<TransactionResponse> GetById(int id)
    {
        try
        {
            var entity = unitOfWork.Repository<Data.Transaction>().GetById(id);
            if (entity is null)
            {
                return new ApiResponse<TransactionResponse>("Record not found");
            }

            var mapped = mapper.Map<Data.Transaction, TransactionResponse>(entity);
    ...
    public ApiResponse<CashResponse> Withdraw(CashRequest request)
    {
        ...
        Data.Transaction transaction = new();
        ...
        unitOfWork.Repository<Data.Transaction>().Insert(transaction);
        unitOfWork.Complete();
    ...
    public ApiResponse<TransferResponse> Transfer(TransferRequest request)
    {
        ...
        Data.Transaction transactionTo = new();
        ...
        unitOfWork.Repository<Data.Transaction>().Insert(transactionTo);

        Data.Transaction transactionFrom = new();
        ...
        unitOfWork.Repository<Data.Transaction>().Insert(transactionFrom);
    ...
    private Expression<Func<Data.Transaction, bool>> GetExpression(int accountId, int customerId, decimal amount, string description)
    {
        var predicate = PredicateBuilder.New<Data.Transaction>(true);
